...
document.addEventListener( "DOMContentLoaded", function ( event ) {
	...
	console.log( 'EMEA Data layer checkout script loaded successfully' );
	...
	// [4]
	const CHECKOUT_PAYMENT = document.getElementById( 'dr_ShoppingCart' ); // Payment Details Page - EMEA & NAM/APAC share the same ID for the Payment Details view.
	...
	// [5]
	const CHECKOUT_ORDER_SUMMARY = document.getElementById( 'QuickBuyConfirmOrderPage' ); // Order Summary Page
	...
	// [6]
	const CHECKOUT_THANKYOU = document.getElementById( 'ThankYouPage' ); // Order Confirmation Page - EMEA & NAM/APAC share the same ID for the Order Confirmation view.
	...
	function gtmCartView() {
		...
		let listcategories = utag_data["product_category"]; // eslint-disable-line no-undef
		...
		// *** Pass the array of categories into our function to be split up into equal chunks. ***
		let categories    = splitToChunks( listcategories, productCount );
		let categoryIndex = categories;
		...
		for ( var i = 0; i < productCount; ++i ) {
			...
			// *** Create our Category variables. ***
			this[ "marker" + i ] = categoryIndex[ i ];
			this[ "category_string_" + i ] = this[ "marker" + i ].toString();
			this[ "category_Array_" + i ] = {
				id: this[ "category_string_" + i ],
			};
			...
			this[ "product" + i ] = {
				...
				brand: "Dragon",
				category: this[ "category_string_" + i ],
